...
"""Plan one FR3 trajectory with MPD and optionally send it through EM to JTC.

The ROS-facing request, saved result, and published trajectory use FR3 names.
The safe default is ``plan_only:=true``: inference and validation run, but no
robot command is published. The current checkpoint continues to use its
...
class SendMpdTrajectory(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("wait_for_interfaces_s", 15.0)
        ...
        self.wait_for_interfaces_s = float(
            self.get_parameter("wait_for_interfaces_s").value
        )
    ...
    def _on_status(self, msg: GoalStatusArray) -> None:
        ...
        if GoalStatus.STATUS_EXECUTING in statuses:
            self._saw_executing = True
        if self._saw_executing and GoalStatus.STATUS_SUCCEEDED in statuses:
    ...
    def _tick(self) -> None:
        ...
        if self._plan_result is None:
            ...
            if self.plan_only:
                self.get_logger().info("PLAN ONLY: trajectory was not published")
                self._finish(0)
                return
        ...
        if self._published_at is None:
            ...
            if not self._interfaces_ready():
                ...
                if now - self._ready_since > self.wait_for_interfaces_s:
                    self.get_logger().error(
                        "Timeout waiting for EM subscription or JTC status"
                    )
                    self._finish(1)
                return
            if self.state is None or not self._joint_state_is_fresh(now):
            ...
            self.get_logger().info(
                f"Published MPD plan with {len(message.points)} points"
            )
            return

        if self._saw_succeeded:
            self.get_logger().info("PASS: JTC reported EXECUTING then SUCCEEDED")
            self._finish(0)
            return
        ...
        if now - self._published_at > result_timeout:
            self.get_logger().error(
                f"No JTC success within {result_timeout:.1f}s "
