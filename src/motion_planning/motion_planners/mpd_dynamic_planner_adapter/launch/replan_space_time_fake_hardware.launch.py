"""Phase-5 space-time replanning with the Franka server on fake hardware."""
...
def generate_launch_description() -> LaunchDescription:
    ...
    space_time_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
                    FindPackageShare("mpd_dynamic_planner_adapter"),
                    "launch",
                    "replan_space_time.launch.py",
                ]
            )
        ),
        launch_arguments={
    ...
    demo = Node(
        ...
        name="mpd_space_time_world_demo",
        output="screen",
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "socket_path", default_value="/tmp/mpd-space-time-runtime.sock"
            ),
            ...
            server,
            TimerAction(period=5.0, actions=[demo, space_time_launch]),
        ]
    )
